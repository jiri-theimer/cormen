...
namespace BL
{
    ...
    class CBL : ICBL
    {
        ...
        public string DeleteRecord(string entity,int pid)
        {
            ...
            pars.Add("userid", AppRunning.Get().UserID, System.Data.DbType.Int32);
        ...
        {     
            if (_userparams == null)
            {
                _userparams= DL.DbHandler.GetList<BO.COM.StringPairValue>("SELECT x36Key as [Key],x36Value as [Value] FROM x36UserParam WHERE j02ID=@j02id", new { j02id = AppRunning.Get().UserID });
            }
